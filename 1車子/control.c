...
void Init_GPIO()
{
	...
	DrvGPIO_Open(E_GPD, 5, E_IO_INPUT); //避障
	
	DrvGPIO_Open(E_GPD, 6, E_IO_OUTPUT); //wifi_reset
	DrvGPIO_SetBit(E_GPD, 6);           //永遠high
	DrvGPIO_Open(E_GPD, 8, E_IO_OUTPUT); //右馬達
	DrvGPIO_Open(E_GPD, 9, E_IO_OUTPUT); //左馬達
	DrvGPIO_ClrBit(E_GPD, 8);           //初始停止
...
int32_t main()
{
	...
	DrvGPIO_ClrBit(E_GPD, 6); //wifi初始化
	DrvSYS_Delay(10000);
	Init_GPIO();
	ADC_initialize();
	delay_s(5);
