...
int main (void)
{
	...
	/*		writeByte(0x68, 0x37,0x02);
    	...
    	hz = (float)magCount[2]*1.49939*magCalibration[2]-15.5f;		//25.5 home	15school
	...
	/*	if(hx>maxx)
		...
		sprintf(TEXT2,"%f ",hy);	
		print_lcd(0,TEXT2);	
		sprintf(TEXT2,"sr%ft%f",hy-hyy,hz-hzz);		  
	...
	/*	if(roll<0.5f && roll>-0.5f &&az>0)
		...
		/////////////////////////////////////////////  
	/*	sprintf(TEXT2,"%f %f",yaw,mi);		  
		print_lcd(3,TEXT2);									
	//	DrvUART_Write(UART_PORT0,textout,strlen(textout));  
		sprintf(textout,"ar%frp%fpy%fy",roll,pitch,(yaw-0.45f)*4);   		
		DrvUART_Write(UART_PORT0,textout,strlen(textout));  							   
		DrvSYS_Delay(2000);			  */
			}
	   }
