...
void CHIDAPDlg::OnBnClickedButtonBurnAprom()
{
    ...
    {
		MessageBox(_T("No file!"),_T("Error"),MB_OK);
	    return;
    }
	...
	if(pMainDlg->CodeFileBuffer != NULL)
	{
        ...
        if (m_sMyFileInfo.uCodeFileSize % 64 == 0)
        {
            packetCount = m_sMyFileInfo.uCodeFileSize / 64;
            remainByte = 0;
        }
        else
        {
            packetCount = m_sMyFileInfo.uCodeFileSize / 64 + 1;
            remainByte = m_sMyFileInfo.uCodeFileSize % 64;
        }

		//Transfer Data
		if (packetCount > 0)
		{
			//1. CMD, Download Program
            ...
            for (packetNum = 0; packetNum < packetCount; packetNum++)
            {
                if (remainByte != 0)
                {
                    ...
                    {
						if(io.OpenDevice(0x051A, 0x511B))
							Result = io.WriteDataFile(CMD_NONE, (const char *)pMainDlg->CodeFileBuffer+packetNum*64, m_sMyFileInfo.uCodeFileSize-packetNum*64, 2000);
                    }
                    else
                    {
						if(io.OpenDevice(0x051A, 0x511B))
						{
							Result = io.WriteDataFile(CMD_NONE, (const char *)pMainDlg->CodeFileBuffer+packetNum*64, remainByte, 2000);
							if(Result && (packetNum == (packetCount - 1)))
							{
								// Checksum	(4 bytes) + Packet Number (4 bytes)
								Result = io.ReadFile((char *)pMainDlg->ReadReportBuffer, 64, 2000, true);
								if(!Result)
