...
void ADC_initialize (void)
{
	...
	((GCR_T *) ((( uint32_t)0x50000000) + 0x00000))->GPAMFP.ADC1_AD12=1; 		
...
void MadgwickQuaternionUpdate(float ax, float ay, float az, float gx, float gy, float gz, float mx, float my, float mz)
{
	...
	float zeta = sqrt(3.0f / 4.0f) * 3.14159265358979323846f* (1.0f / 180.0f);  
	...
	*((volatile uint32_t *)(((( uint32_t)0x50000000) + 0x00000) + 0x100)) = 0x00;;
	...
	
	{
		...
		while(1){
			...
			hx = (float)magCount[0]*1.49939*magCalibration[0]-50.0f;	   
			...
			Xh = (float)hx * cos(pitch) + (float)hy * sin(roll)*sin(pitch) - (float)hz * sin(roll)*sin(pitch);
			...
			if((int)(((ADC_T *) ((( uint32_t)0x40000000) + 0xE0000))->ADDR[7].RSLT)>=1500)
				ad=1;
			else 
				ad=0;		
			sprintf(TEXT2,"%f ",yaw);	
			print_lcd(0,TEXT2);	
			if((int)yaw>=0 && (int)pitch>=0)
				sprintf(TEXT2,"s%d%d%d",(int)yaw,(int)pitch,ad);	
			else if((int)yaw<0 && (int)pitch>=0)
				sprintf(TEXT2,"sn%d%d%d",-(int)yaw,(int)pitch,ad);	
			else if((int)yaw>=0 && (int)pitch<0)
				sprintf(TEXT2,"s%dn%d%d",(int)yaw,-(int)pitch,ad);
			else
				sprintf(TEXT2,"sn%dn%d%d",-(int)yaw,-(int)pitch,ad);		  
