...
int main (void)
{
	...
	//while(1)
	{
		...
		initMPU9250();
		initAK8963(magCalibration);		//阀1.2
		DrvSYS_Delay(300000);		
		...
		while(1){
			...
			hx = (float)magCount[0]*1.49939*magCalibration[0]-50.0f;	   //mag value	     -50		   厩-55
			...
			/*	yaw   = atan2(2.0f * (q[1] * q[2] + q[0] * q[3]), q[0] * q[0] + q[1] * q[1] - q[2] * q[2] - q[3] * q[3]);//熬耚  
			pitch = -asin(2.0f * (q[1] * q[3] - q[0] * q[2]));														 //ヵ
			roll  = atan2(2.0f * (q[0] * q[1] + q[2] * q[3]), q[0] * q[0] - q[1] * q[1] - q[2] * q[2] + q[3] * q[3]);//陆簎
			pitch *= 180.0f / PI;
			yaw   *= 180.0f / PI; 
