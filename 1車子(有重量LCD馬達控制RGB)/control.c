...
void Init_GPIO()
{
	...
	//馬達主開關
	DrvGPIO_Open(E_GPB, 9, E_IO_OUTPUT); //馬達    pb.9
	DrvGPIO_ClrBit(E_GPB, 9);           //初始停止
	DrvGPIO_Open(E_GPB, 10, E_IO_OUTPUT); //馬達    pb.10
	DrvGPIO_ClrBit(E_GPB, 10);           //初始停止
...
void RGB_value()
{
	...
	RGB_count=0;
	DrvGPIO_SetBit(E_GPC, 2); //10 clear
	DrvGPIO_ClrBit(E_GPC, 3);
	DrvSYS_Delay(10000);
}
