...
{
	DrvADC_Open(ADC_SINGLE_END,ADC_SINGLE_CYCLE_OP,0x1F,EXTERNAL_12MHZ,0x9);   //0xE0 adc7,6,5    0x1F adc0,1,2,3,4
	DrvSYS_Delay(100); 
}
...
int main (void)
{
	...
	DrvGPIO_Open(E_GPC, 12, E_IO_INPUT); // GPC12 pin set to output mode
	DrvGPIO_Open(E_GPC, 13, E_IO_INPUT); // GPC12 pin set to output mode
	DrvGPIO_Open(E_GPC, 11, E_IO_INPUT); // GPC12 pin set to output mode
	...
	while(1)
	{
		...
		hx = (float)magCount[0]*1.49939*magCalibration[0]-40.0f;	   //mag value
		...
		{
			DrvUART_Write(UART_PORT0,"8",1);    //single
		...
		else// if(DrvGPIO_GetBit(E_GPC, 11)==1)
		{
			if(e==0)
			{
				DrvUART_Write(UART_PORT0,"9",1);    //mult
				DrvUART_Write(UART_PORT1,"m",1);    //mult
				e=1;
			}
			if(flag==1)
			{
