...
void UART_INT_HANDLE1(void)
{
	uint8_t i;
	while(((UART_T *) ((( uint32_t)0x40100000) + 0x50000))->ISR.RDA_IF==1) 
	{
		DrvUART_Read(UART_PORT1,bInChar1,1);	
		DrvUART_Write(UART_PORT0,bInChar1,1);	
		DrvSYS_Delay(100000); 
	}
}
void ADC_initialize (void)
{
	DrvADC_Open(ADC_SINGLE_END,ADC_SINGLE_CYCLE_OP,0x1F,EXTERNAL_12MHZ,0x9);   
	DrvSYS_Delay(100); 
}
...
int main (void)
{
	...
	int ud,rl;
	int e=0;  
	int32_t adc1=3000,adc2=3000,adc3=3000;
	...
	*((volatile uint32_t *)(((( uint32_t)0x50000000) + 0x00000) + 0x100)) = 0x00;;
	...
	DrvUART_EnableInt(UART_PORT1, DRVUART_RDAINT, UART_INT_HANDLE1); 
	resetMPU9250(); 
	...
	while(1)
	{
		...
		gx = gyroCount[0]*250.0/32768.0-gyroBias[0];		 
		gy = gyroCount[1]*250.0/32768.0-gyroBias[1]; 
		gz = gyroCount[2]*250.0/32768.0-gyroBias[2]; 			
		hx = (float)magCount[0]*1.49939*magCalibration[0]-40.0f;	   
		hy = (float)magCount[1]*1.49939*magCalibration[1]-105.0f; 
		hz = (float)magCount[2]*1.49939*magCalibration[2]+15.5f;	   
		if(DrvGPIO_GetBit(E_GPC, 13)==1)
		{
			DrvUART_Write(UART_PORT0,"8",1);    
			DrvUART_Write(UART_PORT1,"s",1);    
			if(flag==1)
			{
				if(ax>0.7)
					sprintf(textout,"x1");
		...
		else
		{
			if(e==0)
			{
				DrvUART_Write(UART_PORT0,"9",1);    
				DrvUART_Write(UART_PORT1,"m",1);    
				e=1;
			}
			if(flag==1)
			{
