...
// At end of sample accumulation, turn off FIFO sensor read
  ...
  for (ii = 0; ii < packet_count; ii++) {
    ...
    data[0]=readByte(MPU,FIFO_R_W); 
	data[1]=readByte(MPU,FIFO_R_W);
	data[2]=readByte(MPU,FIFO_R_W);
	data[3]=readByte(MPU,FIFO_R_W);
	data[4]=readByte(MPU,FIFO_R_W);
	data[5]=readByte(MPU,FIFO_R_W);
	data[6]=readByte(MPU,FIFO_R_W);
	data[7]=readByte(MPU,FIFO_R_W);
	data[8]=readByte(MPU,FIFO_R_W);
	data[9]=readByte(MPU,FIFO_R_W);
	data[10]=readByte(MPU,FIFO_R_W);
	data[11]=readByte(MPU,FIFO_R_W);
...
int main (void)
{
	...
	/*		writeByte(0x68, 0x37,0x02);
		...
		ax = accelCount[0]- accelBias[0];	  //accel  value
		ay = accelCount[1]- accelBias[1]; 
    	az = accelCount[2]- accelBias[2];  	  
		gx = gyroCount[0];		 //gyro  value
		gy = gyroCount[1]; 
    	gz = gyroCount[2]; 			
		hx = (float)magCount[0]*1.49939*magCalibration[0];	   //mag value
