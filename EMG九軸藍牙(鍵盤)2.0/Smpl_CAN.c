	...
	//while(1)
	{
		...
		while(1){
			...
			*/
			...
			sprintf(TEXT2,"%f ",pitch);		  
			print_lcd(0,TEXT2);	
			sprintf(TEXT2,"%f ",roll);		  
			print_lcd(1,TEXT2);	
			sprintf(TEXT2,"%f ",yaw);		  
			print_lcd(2,TEXT2);	   
			if(yaw>mx)
			...
			if(adc>=2000)
			{
				DrvUART_Write(UART_PORT0,"s55",2);  
				DrvSYS_Delay(20000);	
			}
			...
			if(pitch>=0.5f)
				...
				ud=0;
			if(adc<2000 && roll<0.7 && roll>-0.7 && pitch<0.5 && pitch>-0.5)
			{
				rl=0;
				ud=0;
			}	
			sprintf(textout,"s%d%d",rl,ud);	
			DrvUART_Write(UART_PORT0,textout,3);
			///////////////////////////////////////////
		//	sprintf(textout,"s%d",adc);		
			//DrvUART_Write(UART_PORT0,textout,strlen(textout));  
			print_lcd(3,textout);							   
			DrvSYS_Delay(2000);	

			///////////////測試手部姿勢/////////////////
			/*if(roll<0.5f && roll>-0.5f &&az>0)
			print_lcd(0,"up");
			else if(roll<0.5f && roll>-0.5f &&az<0)
			print_lcd(0,"down");	
			else if(roll<-1.0f )
			{
			print_lcd(0,"right");
			if(yaw>0.0f && yaw<1.5f)
			print_lcd(1,"rightleft");	
			else if(yaw<0.0f && yaw>-1.5f)
			print_lcd(1,"rightright");	
			else
			print_lcd(1,"center");
			}	
			else if(roll>1.0f)
			...
			print_lcd(0,"left");
			if(yaw>0.0f && yaw<1.5f)
			print_lcd(1,"leftleft");	
			else if(yaw<0.0f && yaw>-1.5f)
			print_lcd(1,"leftright");
			else
			print_lcd(1,"center");
			}	  		*/						  
