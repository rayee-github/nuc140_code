...
{
    {CTRL_EP_NUM   		| EP_INPUT , HID_MAX_PACKET_SIZE_CTRL   , NULL},   /* EP Id 0, EP Addr 0, input , max packe size = 8 */ 
    {CTRL_EP_NUM  		| EP_OUTPUT, HID_MAX_PACKET_SIZE_CTRL   , NULL},   /* EP Id 1, EP Addr 0, output, max packe size = 8 */ 
    {INT_IN_EP_NUM_MS | EP_INPUT , HID_MAX_PACKET_SIZE_INT_IN , NULL},   /* EP Id 2, EP Addr 1, input , max packe size =  64 */ 
		{INT_IN_EP_NUM_KB | EP_INPUT , HID_MAX_PACKET_SIZE_INT_IN , NULL},   /* EP Id 2, EP Addr 1, input , max packe size =  64 */
    {0x00,  0, NULL}
};
...
S_DRVUSB_EVENT_PROCESS g_sUsbOps[12] = 
{
    ...
    {HID_IntInCallback_MS   , &g_HID_sDevice},/* EP address 1 In ACK callback */
    {NULL, NULL},                             /* EP address 1 Out ACK callback */
    {HID_IntInCallback_KB		, &g_HID_sDevice},/* EP address 2 In ACK callback */  
...
static void HID_Reset(S_HID_DEVICE *psDevice)
{   
    DrvUSB_Reset(INT_IN_EP_NUM_MS);
		DrvUSB_Reset(INT_IN_EP_NUM_KB);
}
...
void HID_CtrlSetupSetReport(void * pVoid)
{
    ...
    else if (psDevice->au8Setup[3] == 2)
    {
        ...
        //_DRVUSB_TRIG_EP(1, 0x00);
				DrvUSB_DataOutTrigger(CTRL_EP_NUM, HID_MAX_PACKET_SIZE_CTRL);
...
void HID_CtrlSetupDataOutReport(void * pVoid)
{
	...
	pu8EpBuf = DrvUSB_GetOutData(CTRL_EP_NUM, &size);
...
void HID_CtrlSetupGetDescriptor(void * pVoid)
{
    ...
    case DESC_HID_RPT:
    {
        if (pUsbDevice->au8Setup[4] == 0x0)     /* mouse */
					HID_PrepareDescriptors(g_HID_sDevice.pu8ReportDescriptor_MS, g_HID_sDevice.u32ReportDescriptorSize_MS, u16Len, HID_MAX_PACKET_SIZE_CTRL);
				if (pUsbDevice->au8Setup[4] == 0x1)     /* keyboard */
					HID_PrepareDescriptors(g_HID_sDevice.pu8ReportDescriptor_KB, g_HID_sDevice.u32ReportDescriptorSize_KB, u16Len, HID_MAX_PACKET_SIZE_CTRL);
        break;
    }
