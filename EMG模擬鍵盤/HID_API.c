...
void HID_SetInReport()
{
 ...
 uint8_t *buf;
    ...
    key = (GPIOD->PIN & (1 << 2))? 0 : 1;
		...
		/* If GPD3 = 0, just report it is key "Volume +" */
    key1 = (GPIOD->PIN & (1 << 3))? 0 : 1;
		
		/* If GPD4 = 0, just report it is key "Volume -" */
    key2 = (GPIOD->PIN & (1 << 4))? 0 : 1;
...
int32_t HID_MainProcess(void)
{
    ...
    while(1)
    {
       ...
       // PowerDown();
        ...
        while(1)
        {	
			    ADC_result = get_ADC_value(1); 					   //1
				//uint16_ascii(adc_value,ADC_result);
...
/*
void uint16_asciii(uint16_t value,char text[])
{ 
	uint8_t d1,d0;
	int8_t ia=5;
	for (ia=4; ia>0; ia--)
