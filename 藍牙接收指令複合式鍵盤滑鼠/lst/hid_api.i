...
const uint32_t g_HID_u32MouseReportDescriptorSize = sizeof (g_HID_au8MouseReportDescriptor) / sizeof(g_HID_au8MouseReportDescriptor[0]);

uint8_t g_au8MouseReport[3];
...
const uint32_t g_HID_u32KeyboardReportDescriptorSize = sizeof(g_HID_au8KeyboardReportDescriptor) / sizeof(g_HID_au8KeyboardReportDescriptor[0]);

uint8_t g_au8KeyboardReport[9];
...
const uint8_t g_HID_au8ConfigDescriptor[] =
{
    ...
    (9+9*2+0x09*2+7*2) & 0x00FF,
    ((9+9*2+0x09*2+7*2) & 0xFF00) >> 8,
    0x02,           
    0x01,           
    0x00,           
    0x80 | (0 << 6) | (1 << 5),
    ...
    sizeof (g_HID_au8MouseReportDescriptor) / sizeof(g_HID_au8MouseReportDescriptor[0]) & 0x00FF,
    (sizeof (g_HID_au8MouseReportDescriptor) / sizeof(g_HID_au8MouseReportDescriptor[0]) & 0xFF00) >> 8,
    ...
    sizeof(g_HID_au8KeyboardReportDescriptor) / sizeof(g_HID_au8KeyboardReportDescriptor[0]) & 0x00FF,
    (sizeof(g_HID_au8KeyboardReportDescriptor) / sizeof(g_HID_au8KeyboardReportDescriptor[0]) & 0xFF00) >> 8,
...
void HID_SetInReport_MS()
{
    ...
    g_HID_sDevice.isReportReady_MS = 1;  
    DrvUSB_DataIn(0x01, g_HID_sDevice.pu8Report_MS, g_HID_sDevice.u32ReportSize_MS);
}
   ...
   if (key == 0)
	{
		...
		if((key != preKey))
		{
	       ...
	       g_HID_sDevice.isReportReady_KB = 1;
            DrvUSB_DataIn(0x02, g_HID_sDevice.pu8Report_KB, g_HID_sDevice.u32ReportSize_KB);
  		}
	}
	else if (key == 1)
	{	
...
void PowerDown()
{
    ...
    *((volatile uint32_t *)(((( uint32_t)0x50000000) + 0x00000) + 0x100)) = 0x59;*((volatile uint32_t *)(((( uint32_t)0x50000000) + 0x00000) + 0x100)) = 0x16;*((volatile uint32_t *)(((( uint32_t)0x50000000) + 0x00000) + 0x100)) = 0x88;
    ...
    ((USBD_T *) ((( uint32_t)0x40000000) + 0x60000))->INTEN.WAKEUP_EN = 1;

     
    *((volatile unsigned int *)(0xE000ED10))=4;
    ((SYSCLK_T *) ((( uint32_t)0x50000000) + 0x00200))->PWRCON.PD_WAIT_CPU = 1;
    ...
    if(DrvUSB_GetUsbState() & eDRVUSB_SUSPENDED)
    {
        ...
        if(DrvUSB_IsRemoteWakeupEnabled())
        {
            ...
            ((USBD_T *) ((( uint32_t)0x40000000) + 0x60000))->ATTR.PHY_EN = 1;

             
            ((USBD_T *) ((( uint32_t)0x40000000) + 0x60000))->ATTR.RWAKEUP = 1;
            DrvSYS_Delay(1000);  
            ((USBD_T *) ((( uint32_t)0x40000000) + 0x60000))->ATTR.RWAKEUP = 0;
        }
    }
...
void UART_INT_HANDLE(void)
{
	...
	while(((UART_T *) ((( uint32_t)0x40000000) + 0x50000))->ISR.RDA_IF==1) 
	{
		...
		if(bInChar[0]=='s') 
		{
			...
			while(bInChar[0]!='a')
			{
				comRbuf2[i] = bInChar[0];
				i++;	
				DrvUART_Read(UART_PORT0,bInChar,1);	
			}
			DrvUART_Read(UART_PORT0,bInChar,1);	
			comRbuf3[0] = bInChar[0];
			i=0;
			break;
		}
	}
...
int32_t HID_MainProcess(void)
{
    ...
    ((GPIO_T *) (((( uint32_t)0x50000000) + 0x4000) + 0x00C0))->IEN = (3 << 1);
