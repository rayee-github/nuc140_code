...
public:
	...
	BOOL WriteFile(unsigned long uCmd, const char *pcBuffer, DWORD dwAddr, DWORD dwLen, DWORD dwMilliseconds)
	{
		...
		if(dwCmdLength > sizeof(m_acBuffer) - 17)
			dwCmdLength = sizeof(m_acBuffer) - 17;
		...
		if(pcBuffer != NULL && dwCmdLength > 0)
			memcpy(m_acBuffer + 17, pcBuffer, dwCmdLength);

		m_usCheckSum = Checksum((unsigned char *)&m_acBuffer[17], dwCmdLength);
	...
	BOOL WriteDataFile(unsigned long uCmd, const char *pcBuffer, DWORD dwLen, DWORD dwMilliseconds)
	{
		...
		if(dwDataLength > sizeof(m_acBuffer) - 9)
			dwDataLength = sizeof(m_acBuffer) - 9;
		...
		//*((ULONG *)&m_acBuffer[9]) = dwAddr;
		//*((ULONG *)&m_acBuffer[13]) = dwCmdLength;

		if(pcBuffer != NULL && dwDataLength > 0)
			memcpy(m_acBuffer + 9, pcBuffer, dwDataLength);

		dwDataCheckSum = Checksum((unsigned char *)&m_acBuffer[9], dwDataLength);
