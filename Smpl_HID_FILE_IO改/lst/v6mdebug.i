...
void V6M_ProcessCommand(const uint8_t *pu8Buffer)
{
    static uint8_t au8RetBuffer[64], au8DataBuffer[56]; 
	static uint8_t au8SPIWriteDataBuffer[256], au8SPIReadDataBuffer[256];
	uint32_t i;
	uint32_t u32Cmd;
	((USBD_T *) ((( uint32_t)0x40000000) + 0x60000))->INTEN . USB_IE = 0;
    ...
    for (i = 0;i < 56;i++)
	{
        au8DataBuffer[i] = 0x00; 
	}

    for (i = 0;i < 256;i++)
	{
        au8SPIWriteDataBuffer[i] = 0x00; 
		au8SPIReadDataBuffer[i] = 0x00; 
	}
...
#line 226 "V6MDebug.c"
	...
	((USBD_T *) ((( uint32_t)0x40000000) + 0x60000))->INTEN . USB_IE = 1;
}
