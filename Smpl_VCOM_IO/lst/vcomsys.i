...
enum {VCOM_VERSION_NUM = ((1) << 16) | ((0) << 8) | (1)};
...
int32_t VCOM_Open(void *pfGetInData, void *pfSetOutData);
...
S_DRVUSB_EVENT_PROCESS g_sBusOps[6] = 
{
	{0, 0},                                
	{0, 0},                                
	{DrvUSB_BusResetCallback, &gsVcomDevice},    
	{0, 0},                                
	{0, 0},                                
	{DrvUSB_CtrlSetupAck, &gsVcomDevice},        
};
...
S_DRVUSB_CTRL_CALLBACK_ENTRY g_asCtrlCallbackEntry[] = {
	...
	{0x00, 0x05, DrvUSB_CtrlSetupSetAddress, DrvUSB_CtrlDataInSetAddress, 0, &gsVcomDevice},
	{0x00, 0x01, DrvUSB_CtrlSetupClearSetFeature, 0, 0, &gsVcomDevice},
	{0x00, 0x03, DrvUSB_CtrlSetupClearSetFeature, 0, 0, &gsVcomDevice},
	{0x00, 0x08, DrvUSB_CtrlSetupGetConfiguration, 0, 0, &gsVcomDevice},
	{0x00, 0x00, DrvUSB_CtrlSetupGetStatus, 0, 0, &gsVcomDevice},
	{0x00, 0x0A, DrvUSB_CtrlSetupGetInterface, 0, 0, &gsVcomDevice},
	{0x00, 0x0B, DrvUSB_CtrlSetupSetInterface, 0, 0, &gsVcomDevice},
	{0x00, 0x06, VCOM_CtrlSetupGetDescriptor, VCOM_CtrlDataInGetDescriptor, 0, &gsVcomDevice},
	{0x00, 0x09, VCOM_CtrlSetupSetConfiguration, 0, 0, &gsVcomDevice},
	{0x20, 0x20, VCOM_SetLineCoding, 0, VCOM_SetLineCodingDataOut, &gsVcomDevice},
    {0x20, 0x21, VCOM_GetLineCoding, 0, 0, &gsVcomDevice}, 
    {0x20, 0x22, VCOM_CtrlLineState, 0, 0, &gsVcomDevice}
};


 
void (*pfHID_GetInData)(uint8_t *buf) = 0;

 
void (*pfHID_SetOutData)(uint8_t *buf) = 0;
...
S_VCOM_LINE_CODING gLineCoding = {115200, 0, 0, 8};     
...
void VCOM_CtrlSetupGetDescriptor(void* pVoid)
{
	...
	switch (psDevice->au8Setup[3])
	{
		...
		case 0x02:
		{
			...
			gu16TransferLen = ((u16Len)<(u16ConfigLen) ? (u16Len) : (u16ConfigLen));
			gu16PacketLen = 64;
			gu16TransferIndex = 0;		
		...
		case 0x03:
		{
			...
			else
			{
				...
				{
					case 1:
						u16Len = ((u16Len)<(psVcomDevice ->sVendorStringDescriptor[0]) ? (u16Len) : (psVcomDevice ->sVendorStringDescriptor[0]));
				
						DrvUSB_DataIn(0, (const uint8_t *)psVcomDevice->sProductStringDescriptor, u16Len);
						break;
					case 2:
						u16Len = ((u16Len)<(psVcomDevice ->sProductStringDescriptor[0]) ? (u16Len) : (psVcomDevice ->sProductStringDescriptor[0]));
						DrvUSB_DataIn(0, (const uint8_t *)psVcomDevice->sProductStringDescriptor, u16Len);
						break;
					case 3:
						u16Len = ((u16Len)<(gau8StringSerial[0]) ? (u16Len) : (gau8StringSerial[0]));
...
int32_t VCOM_Open(void *pfGetInData, void *pfSetOutData)
{
    ...
    if((uint32_t)pfGetInData != 0)
        pfHID_GetInData = (void(*)(uint8_t *))pfGetInData;
        
    if((uint32_t)pfSetOutData != 0)
        pfHID_SetOutData = (void(*)(uint8_t *))pfSetOutData;
...
void VCOM_BulkOutAckCallback(void* pVoid)
{
    ...
    if ((uint32_t)pfHID_SetOutData)
        pfHID_SetOutData(pu8EpBuf);
...
void VCOM_BulkInAckCallback(void* pVoid)
{
    S_VCOM_DEVICE* psDevice = (S_VCOM_DEVICE*) pVoid;

    if ((uint32_t)pfHID_GetInData)
        pfHID_GetInData(psDevice->pu8Data);
