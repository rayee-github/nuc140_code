...
static int32_t ADC_average (int32_t *ADC_value)  //加static表示?本文件?部用
{
	int32_t value[9] = {0};
	uint8_t i,j,k;
	int32_t value_max;
	for (i=0; i<9; i++)
	{
		value[i] = *ADC_value++;                    //?取?要??的值
	}

	//冒泡法?小到大排列
	for (j=0; j<9; j++)                    //一共要比?9次
	{
		for (k=0; k<9-j; k++)      //第一次?后最大的值放在了最高的位置
		{           //就不用再比?最大的那?元素了
			if(k != 8)        //防止k?8?，value[8]与value[9]的比?，因?后者已?超出了??的范?
			{
				if (value[k] > value[k+1])      //所以每次比?完下一次少比?一次
				{
					value_max = value[k];   //??大的值保存起?
					value[k] = value[k+1];   //?小的值??低位元素
					value[k+1] = value_max;   //?大的值??高位元素
				}
			}
		}
	}
...
{
	uint8_t i;
	int32_t ADC_calibration[9]={0};                                   //ADC求中?值?据?存??
	for (i=0; i<9; i++)                      //?取9次的值
	{
		DrvADC_StartConvert();                    //??ADC??
		while(!DrvADC_IsConversionDone())                 //等待ADC??完成
		{
		}
		ADC_calibration[i] = DrvADC_GetConversionData(ADC_channel_number); //?取??ADC通道的值
		DrvADC_StopConvert();                                           //停止??
	}
	return (ADC_average(ADC_calibration));                 //?取到的值求取中?值后返回
}
...
int main (void)
{
	...
	float aax,aay,aaz,axx,ayy,hyy,hzz;
	int flg=0;
	float temp;
	float Xh,Yh	;
	float mx=0.0f,mi=0.0f;
	float maxx=0.0f,minx=100.0f ,maxy =0.0f,miny=100.0f ,maxz=0.0f,minz=110.0f;
	...
	Initial_pannel();
	clr_all_pannal();
	sprintf(TEXT2,"%d ",ADC->ADDR[7].RSLT );
	show_string(2,10,TEXT2);
	DrvSYS_Delay(200000);
	ADC->ADCR.ADST=1;	
	}	 */ 
	//////////////////////////////////////				
	...
	//while(1)
	{
		...
		initMPU9250();
		initAK8963(magCalibration);		//大概1.2
		DrvSYS_Delay(300000);		
		...
		while(1){
			...
			hx = (float)magCount[0]*1.49939*magCalibration[0]-50.0f;	   //mag value	     -50		   學校-55
			hy = (float)magCount[1]*1.49939*magCalibration[1]-105.0f; 					   //-105			  -105
			hz = (float)magCount[2]*1.49939*magCalibration[2]-25.5f;					   //-25.5				 0
			//	MahonyQuaternionUpdate(ax, ay, az, gx*PI/180.0f, gy*PI/180.0f, gz*PI/180.0f,  hx,  hy, hz);
			/*	yaw   = atan2(2.0f * (q[1] * q[2] + q[0] * q[3]), q[0] * q[0] + q[1] * q[1] - q[2] * q[2] - q[3] * q[3]);//偏擺  
			pitch = -asin(2.0f * (q[1] * q[3] - q[0] * q[2]));														 //俯仰
			roll  = atan2(2.0f * (q[0] * q[1] + q[2] * q[3]), q[0] * q[0] - q[1] * q[1] - q[2] * q[2] + q[3] * q[3]);//翻滾
			pitch *= 180.0f / PI;
			yaw   *= 180.0f / PI; 
			yaw   -= 13.8f; // Declination at Danville, California is 13 degrees 48 minutes and 47 seconds on 2014-04-04
			roll  *= 180.0f / PI;		 */
			roll = atan2(-ax,(sqrt(az * az + 0.1 * ay * ay))); // mu = 0.1 and sign = +1 for positive az2 and -1 for negative
		...
		/*			if(hx>maxx)
			...
			if(hz<minz)
			minz=hz;
			sprintf(TEXT2,"%f %f",maxx,minx);		  
			print_lcd(0,TEXT2);	
			sprintf(TEXT2,"%f %f",maxy,miny);  
			print_lcd(1,TEXT2);	
			sprintf(TEXT2,"%f %f",maxz,minz);		  
			print_lcd(2,TEXT2);			*/
			

			sprintf(TEXT2,"s%d ",(int)(ax*10));	  	  
			//print_lcd(0,TEXT2);	
			//sprintf(TEXT2,"s%dt%dx",(int)(ax*30),(int)(ay*30));		
			//DrvUART_Write(UART_PORT0,TEXT2,10);	  
			//print_lcd(1,TEXT2);	
			sprintf(TEXT2,"%f%f ",hy,hz);		  
			//print_lcd(2,TEXT2);	   
			if(yaw>mx)
				mx=yaw;
			if(yaw<mi)
			...
			//sprintf(TEXT2,"%f %f",mx,mi);		  
			//	DrvUART_Write(UART_PORT0,textout,strlen(textout));  
			//sprintf(textout,"ar%frp%fpy%fy",roll,pitch,(yaw-0.45f)*4);   	//給手機的	
			//sprintf(textout,"s%f %f %f %d",ax,ay,az,ADC->ADDR[7].RSLT);	
			//////////////測試移動軌跡///////////////
			if(flg==0)
			{
				hyy=hy;
			...
			sprintf(TEXT2,"%f ",hy);	
			print_lcd(0,TEXT2);	
			sprintf(TEXT2,"s%dt%dx",(int)((hy-hyy)/2),(int)((hz-hzz)/2));		  
			DrvUART_Write(UART_PORT0,TEXT2,strlen(TEXT2));
			print_lcd(1,TEXT2);	
			sprintf(TEXT2,"t%f",hz);	 
			print_lcd(2,TEXT2);		
			sprintf(TEXT2,"%f",ayy);		  
			print_lcd(3,TEXT2);	
			axx=0;
		...
		//////////////////////////////////////////		
			/*	sprintf(textout,"s%d",ADC->ADDR[7].RSLT);		
			DrvUART_Write(UART_PORT0,textout,strlen(textout));  
			print_lcd(0,textout);		*/					   
			DrvSYS_Delay(20000);	
